...
const EmptyFunction = Object.getPrototypeOf(Function);
...
export class ClassProvider {
    ...
    for (const param of params) {
      if (param.token === SuperConstructorAnnotation) {
        ...
        if (SuperConstructor === EmptyFunction) {
          throw new Error(`${toString(constructor)} does not have a parent constructor. ` +
            `Only classes with a parent can ask for SuperConstructor!`);
        }
  ...
  protected createConstructor(currentConstructorIdx: any,
    ...
    return function InjectedAndBoundSuperConstructor(): any {
